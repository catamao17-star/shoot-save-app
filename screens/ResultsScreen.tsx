...
export default function ResultsScreen({ navigation }: Props) {
  ...
  const [reportCount, setReportCount] = useState<number>(currentSession.reportCount ?? 0);
  const [lastReportExportedAt, setLastReportExportedAt] = useState<string | null>(
    currentSession.lastReportExportedAt ?? null
  );
  ...
  useEffect(() => {
    setReportCount(currentSession.reportCount ?? 0);
    setLastReportExportedAt(currentSession.lastReportExportedAt ?? null);
  }, [currentSession]);
  ...
  const handleExportPdf = async () => {
    try {
      ...
      const updatedSession = {
        ...
        lastReportExportedAt: exportedAt,
      };
      ...
      setReportCount(nextReportCount);
      setLastReportExportedAt(exportedAt);

      setIsSyncing(true);
      setSyncError(null);

      const result = await saveSessionToSupabase(updatedSession, completenessScore);

      if (!remoteId && result.remoteId) {
        setRemoteId(result.remoteId);
      }

      Alert.alert('Exported', 'Session report exported and saved to Supabase.');
    } catch (error) {
      const message =
